...
public class StrategyImpl {
	...
	public void iterate(int mins, String dNT) {
		if (indexState.ohlc.time.equals("09:15")) {
...
//	            trade.holdingPeriod++;  // Update current SMA for ongoing trades
//	            
//	        }
		}
...
//			    		    if(indexState.ohlc.date.equalsIgnoreCase("29-04-22") && kv.sno==3) {
//								System.out.println(indexState.ohlc.dnt+"   "+(mins >= kv.startTime )+" "+ dayATRConditionSatisfied
//										+" "+ (checkBollingerBandCondition() )
//										+" "+ (indexState.parser - parserAtLastTrade >= kv.tradeGap)
//										+" "+ (unSquaredTrades <= kv.maxOverLap)
//										+" "+ (toleranceChecker(mins, indexState.ohlc.close, upperBand, lowerBand))
...
//System.out.println(indexState.ohlc.dnt + "  "+entryConditionSatisfied);
...
//			            	System.out.println(indexState.ohlc.dnt);
			                tradeEntities.add(tradeEntity);
			                tradeId++;
			                parserAtLastTrade = indexState.parser;
			                lastTradedMins = indexState.ohlc.mins;
			            }
			        }
//			    }
			} else {
...
//	            if (kv.sno == 1 && indexState.ohlc.date.endsWith("17")) {
