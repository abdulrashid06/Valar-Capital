...
//            System.out.println(closePrice);
        ...
        double sma = sum / actualPeriod;
//        System.out.println("Calculated SMA at index  >>" + endIndex + "  :   " + sma+"  :  "+sum + "  :  "+actualPeriod);
        return sma;
    }
    ...
    public Map<BBType, Double> getBbValues(boolean belongsToLb, int index) {
        if (index >= series.getBarCount() || index < 0) {
            return new HashMap<>();
        }

        Map<BBType, Double> values = belongsToLb ? bbValuesAtLb : bbValues;

        double sma = calculateSMA(index);
        double sd = calculateStandardDeviation(index, sma);
