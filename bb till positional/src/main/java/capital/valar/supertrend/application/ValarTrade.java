...
public class ValarTrade {
    ...
    public static void main(String[] args)throws Exception{
        ...
        for(int indexType : indexTypes) {
            for(boolean pos : positional) {
                ...
                applyStrategyOnKeystore(pos,indexType, keyStoresList.size());
            }
        }
    ...
    private static void applyStrategyOnKeystore(Boolean positional,int bnOrN,int ksSize)throws Exception{
        ...
        for(int i = 0;i < ksSize;i++){
            ...
            for(; runForKeyAttribs.size() < runKeystores && j < ksSize;j++) {
                ...
                if(kv.BnOrN==bnOrN && kv.positional==positional) {
            ...
            if(runForKeyAttribs.size()!=0) {
                Strategy strategy = new Strategy(positional,runForKeyAttribs);
                strategy.apply();
