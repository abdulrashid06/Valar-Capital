...
import static capital.valar.supertrend.utils.Global.strikePlus;
...
//			System.out.println(kv.sno+"   " + indexohlc.dnt + "    "+ indexohlc.close + "    " + currentSma+"   "+sma);
  ...
  // this overallTradeMetric is beign used for the calculation of the day metric, and is updated on exit of trade
    ...
    private void shiftTrade(TradeAttrib ta,String reason,String reasonInfo){
        ta.tradeMetric.calculateOverAllMetricsAndPrint(profitPercentBN.currentProfit, reason, reasonInfo, tradeId, kv, new Ohlc(ta.os.ohlc), indexohlc.close, ta.os.name, printAttribs);
    ...
    private void exitTrade(String reason,String reasonInfo){
        ...
        for(TradeAttrib ta : tradeAttribs){
            ta.tradeMetric.calculateOverAllMetricsAndPrint(profitPercent.currentProfit, reason, reasonInfo, tradeId, kv, new Ohlc(ta.os.ohlc), indexohlc.close, ta.os.name, printAttribs);
        }
        updateOverAll();
        tradeSquared = true;
    }
