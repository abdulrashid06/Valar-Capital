...
//			System.out.println(kv.sno+"   " + indexohlc.dnt + "    "+ indexohlc.close + "    " + currentSma+"   "+sma);
  ...
  // this overallTradeMetric is beign used for the calculation of the day metric, and is updated on exit of trade
    ...
    private void shiftTrade(TradeAttrib ta,String reason,String reasonInfo){
        ta.tradeMetric.calculateOverAllMetricsAndPrint(profitPercentBN.currentProfit,profitCost.currentProfit,profitCostPercent.getTotalProfit() ,reason, reasonInfo, tradeId,holdingPeriod, kv, new Ohlc(ta.os.ohlc), indexohlc.close, ta.os.name, printAttribs);
...
//        	System.out.println(ta);
            ta.tradeMetric.calculateOverAllMetricsAndPrint(profitPercent.currentProfit, profitCost.currentProfit,profitCostPercent.getTotalProfit(), reason, reasonInfo, tradeId,holdingPeriod, kv, new Ohlc(ta.os.ohlc), indexohlc.close, ta.os.name, printAttribs);
        }
        updateOverAll();
        tradeSquared = true;
    }
